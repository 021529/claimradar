"""scripts/sensitivity_analysis.py와 동일한 민감도 분석을, 원래 "16.3%" 개선율이
...
RECOVERY_MULTS = [0.28, 0.34, 0.40, 0.46, 0.52]
HOURS_WEIGHTS = [0.7, 0.85, 1.0, 1.15, 1.3]
COST_PER_HOURS = [245.0, 297.5, 350.0, 402.5, 455.0]


def _load_base_data() -> pd.DataFrame:
    sample = pd.read_csv(SAMPLE_PATH)
    # ML 스코어링 모델 보강(scripts/prepare_app_dataset.py)으로 sample_claims.csv가
    ...
    to_merge = [c for c in needed if c not in sample.columns]
    raw = pd.read_csv(RAW_PATH)[["PolicyNumber", *to_merge]]
    merged = sample.merge(raw, left_on="case_id", right_on="PolicyNumber", how="left") if to_merge else sample
...
def main() -> None:
    ...
    print(
        f"총 사건 {len(merged)}건, 총 조사시간 수요 {total_demand:.0f}시간, "
        f"가용시간 {NUM_INVESTIGATORS * HOURS_PER_INVESTIGATOR}시간\n"
    )
    ...
    results.append(run_scenario(merged, combined_score, "최악조합(0.28/1.3/455)", 0.28, 1.3, 455.0))
    results.append(run_scenario(merged, combined_score, "최선조합(0.52/0.7/245)", 0.52, 0.7, 245.0))
